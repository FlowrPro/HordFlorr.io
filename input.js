...
import dom, { showSkillTooltip, hideSkillTooltip, appendChatMessage } from './dom.js';
import { clampToMap } from './utils.js';
import { sendChat } from './network.js';
import { setComputeInputFunc } from './network.js';
import { castSkill as networkCastSkill } from './cast_stub.js';

// The original castSkill implementation used client-side cooldowns and optimistic visuals
// In the split we keep the same client-side behavior and call the server via network cast (networkCastSkill).
// However, to avoid circular import issues, we include the original castSkill logic here and
// provide a callback hook to send the cast message to the server (network module will provide a sender).
// To keep things simple I adapted the original castSkill and will send the server message using state.ws directly.
...
// Export helpers for main wiring
export {
  initInputHandlers,
  handleHotbarClick,
  getHotbarSlotUnderPointer,
  findEntityUnderPoint,
  computeInputVector
};
...
// Full file: includes redirect/popup handling, forced ID token refresh, and extra logging for debugging.
    ...
    googleSignInBtn.addEventListener('click', async () => {
      try {
        ...
        try {
          ...
          // force fresh token before sending it to server
          const idToken = await user.getIdToken(true);
          console.log('[Auth] popup flow - obtained idToken (len=', idToken.length, ')');
          localStorage.setItem('moborr_idtoken', idToken);
          if (titleScreen) titleScreen.setAttribute('aria-hidden','true');
          connectToServer(idToken);
        } catch (popupErr) {
